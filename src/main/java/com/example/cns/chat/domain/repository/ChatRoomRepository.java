...
public interface ChatRoomRepository extends JpaRepository<ChatRoom, Long> {
    @Query("select room from ChatRoom as room where room.id in (select cp.room from ChatParticipation  as cp where cp.member = :memberId) order by room.lastChatId desc")
