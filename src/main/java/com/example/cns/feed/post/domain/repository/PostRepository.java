...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(value = "SELECT p, " +
            "CASE WHEN (SELECT COUNT(pl) FROM PostLike pl WHERE pl.post.id = p.id AND pl.member.id = :memberId) > 0 THEN true ELSE false END AS userLiked " +
            "FROM Post p " +
            "WHERE p.member.id = :memberId AND " +
            "(p.likeCnt < :likeCnt OR (p.likeCnt = :likeCnt AND p.id < :postId)) " +
            "ORDER BY p.likeCnt DESC, p.createdAt DESC " +
            "LIMIT :pageSize")
    List<Object[]> findPostsByLikeCnt(@Param("memberId") Long memberId, @Param("likeCnt") Long likeCnt, @Param("postId") Long postId, @Param("pageSize") Long pageSize);
}
